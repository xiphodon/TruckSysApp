...
public class HotelDetailActivity extends BaseActivity implements
	...
	private ImageView iv_title, img_shop_tel;
	...
	private ImageView comment, follow;
	...
	private void initView() {
		...
		tv_hotel_name = (TextView) findViewById(R.id.tv_hotel_name);// 宾馆名称
		tv_parsevalue = (TextView) findViewById(R.id.tv_parsevalue);// 宾馆星级
		rb_value = (RatingBar) findViewById(R.id.rb_value);// 宾馆星级
		...
		tv_shop_address = (TextView) findViewById(R.id.tv_shop_address);// 宾馆地址
		ll_to_here= (LinearLayout) findViewById(R.id.ll_to_here);//宾馆地址
		ll_to_here.setOnClickListener(this);
		...
		if (map != null) {
			try {
				String[] picarray = map.get("Hote_Pic").split("\\|");
				...
				tv_hotel_name.setText(map.get("Hote_ShopName"));
				tv_shop_address.setText(map.get("Hote_Address"));
				tv_servicecontent.setText(map.get("Hote_Desc"));
	...
	private void getEvaluationData(int initType, String type) {
		CommentService.getData(type, context, evaluationAdapter, datalist,
				listKey, map.get("Hote_Id"), "Hotel", initType, tv_prase_more,
	...
	public void onClick(View arg0) {
		...
		case R.id.iv_title:
			// 显示更多图片
			Intent imageintent = new Intent(HotelDetailActivity.this,
			...
			break;
		case R.id.ll_to_here:
			// 进入位置导航
			HashMap<String, String> loactionmap = new HashMap<String, String>();
			loactionmap.put("Serv_Latitude", map.get("Hote_Latitude"));
			loactionmap.put("Serv_Longitude", map.get("Hote_Longitude"));
			Intent intentmap = new Intent(HotelDetailActivity.this,
		...
		case R.id.tv_prase_more:
			...
			break;
		case R.id.follow:// 收藏
			FollowService.follow(context, map, "Hote_Id", "Hotel", follow);
			break;
		case R.id.comment:// 评论类型6
			startActivityForResult(
					new Intent(context, CommentActivity.class).putExtra("id",
							map.get("Hote_Id")).putExtra("type", "Hotel"), 100);
			break;
		default:
			break;
		}
