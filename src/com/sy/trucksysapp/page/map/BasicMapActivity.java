...
public class BasicMapActivity extends BaseActivity implements LocationSource,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		topbase_center_text.setText("附近服务商");
	...
	private void showProgressDialog() {
		if (progDialog == null)
			progDialog = CommonUtils.ShowProcess(this, "正在搜索周边附近服务商..");
		progDialog.show();
	}
	...
	public void drawMarkers(ArrayList<HashMap<String, String>> datalist) {
		...
		for (int i = 0; i < datalist.size(); i++) {
			if(type.equals("0")){
				Double lat = Double.valueOf(datalist.get(i).get("Serv_Latitude"));
				Double longit = Double.valueOf(datalist.get(i)
						.get("Serv_Longitude"));
				LatLng latlng = new LatLng(lat, longit);
				shoplist.add(latlng);
			}else if(type.equals("1")){
				Double lat = Double.valueOf(datalist.get(i).get("Deal_Latitude"));
				Double longit = Double.valueOf(datalist.get(i)
						.get("Deal_Longitude"));
				LatLng latlng = new LatLng(lat, longit);
				shoplist.add(latlng);
		...
		for (int i = 0; i < shoplist.size(); i++) {
			if(type.equals("0")){
				...
				.title(datalist.get(i).get("Serv_Name"))
				.snippet(datalist.get(i).get("Serv_Address"))
				.icon(BitmapDescriptorFactory
						.fromResource(R.drawable.icon_mapservice)));
			}else if(type.equals("1")){
				aMap.addMarker(new MarkerOptions()
				.position(shoplist.get(i))
				.title(datalist.get(i).get("Deal_Name"))
				.snippet(datalist.get(i).get("Deal_Address"))
				.icon(BitmapDescriptorFactory
						.fromResource(R.drawable.icon_mapservice)));
			}else{
