 ...
 * 
 */
public class GasMapActivity extends BaseActivity implements LocationSource,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		topbase_center_text.setText("附近加油加气站");
	...
	public void drawMarkers(ArrayList<HashMap<String, String>> datalist) {
		...
		for (HashMap<String, String> map : datalist) {
			Double lat = Double.valueOf(map.get("GaSt_Latitude"));
			Double longit = Double.valueOf(map.get("GaSt_Longitude"));
			LatLng latlng = new LatLng(lat, longit);
			shoplist.add(latlng);
			aMap.addMarker(
					new MarkerOptions()
							.position(latlng)
							.title(map.get("GaSt_ShopName"))
							.snippet(map.get("GaSt_Address"))
							.icon(BitmapDescriptorFactory
									.fromResource(R.drawable.gas_location))).setObject(
					map);
		}
	}
			...
			public void onSuccess(String result) {
				...
				try {
					...
					if(datalist.size()<total){
						...
						drawMarkers(datalist);
						currPage++;
					}else{
						flag=true;
						CommonUtils.showToast(context, "已经加载全部");
					}
//					datalist.addAll(data);
//					drawMarkers(datalist);
					loadstatus = false;
	...
	private void render(Marker marker, View view) {
		...
		HashMap<String, String> map = (HashMap<String, String>) marker
				.getObject();
		String serviceType = map.get("GaSt_ServiceType");
		String text = "加油";
		int color = 0;
		int bg = 0;
		...
		phone.setText(map.get("GaSt_Mobile"));
		address.setText(map.get("GaSt_Address"));
	}
